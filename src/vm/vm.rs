...
impl VM {
    ...
    fn define_global(&mut self) {
        ...
        // Later note: Why was this here in the first place?
        // => Causes issues when doing code like:
        // local function foo() -> Number {...}
        // global a = foo();
        // local function bar() -> Number {...}
        // global b = bar();
        // => As `global a` pushes the return value of foo()
        // on the stack meaning that `bar` is at local index 3
        // instead of 2. The VM will try to then call a Number
        // which doesn't make sense. The VM previously didn't check for
        // this, a new runtime error has been added to notify
        // when the generated bytecoe tries to call a primitive.
        // TODO: Temporary bugfix for function definitions (might be inefficient)
        // self.push(lhs); => the legacy code that led to undefined behavior
    }
